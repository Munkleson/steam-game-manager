  ...
  def callback
    ...
    if params['openid.mode'] == 'id_res' && params['openid.claimed_id'].present?
      ...
      if response.body.include?("is_valid:true")
        ...
        redirect_to add_games_path, notice: "Successfully signed in as Steam user #{steam_id}"
      else
        redirect_to add_games_path, alert: "Steam authentication failed"
      end
    else
      redirect_to add_games_path, alert: "Invalid authentication response"
    end
  end
